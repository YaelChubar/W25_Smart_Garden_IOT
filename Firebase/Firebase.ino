...
// Recommend for ESP8266 stream, adjust the buffer size to match your stream data size
  ...
  if (!Firebase.RTDB.beginStream(&stream, "/gardens"))
...
void loop()
{
  ...
  if (Firebase.ready() && (millis() - sendDataPrevMillis > 15000 || sendDataPrevMillis == 0))
  {
    ...
    moisture_sensor_reading = analogRead(MOISTURE_SENSOR_PIN);
    Serial.print("moisture sensor #1: ");
    Serial.println(moisture_sensor_reading);
    
    //count++; 
    FirebaseJson json;
    json.add("light_sensor_value", calculatePercentage(light_sensor_reading, LIGHT_SENSOR_MIN_VALUE, LIGHT_SENSOR_MAX_VALUE));
    json.add("moisture_sensor_value", moisture_sensor_reading);
    //json.add("num", count);
    //json.add("name", "yael");
    Serial.printf("Set json... %s\n\n", Firebase.RTDB.setJSON(&fbdo, "gardens/garden1/sensors/", &json) ? "ok" : fbdo.errorReason().c_str());
  }
